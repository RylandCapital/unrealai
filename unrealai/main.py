...
def load_csv(csv_path):
    ...
    return pd.read_csv(csv_path, usecols=cols, dtype={c: 'float32' for c in cols})
...
class TradingEnv(gym.Env):
    """Custom trading environment implementing per-step unrealised P&L reward,
    a close-action counter, and a time-decay penalty of –0.002 per step while
    in a position."""
    ...
    def __init__(self,
        ...
        # Store raw df (no copy to save RAM); slice per episode
    ...
    def step(self, action):
        ...
        if   self.position == 1:
            ...
            # FIX: proper short valuation — cash minus current liability
            self.equity = self.cash - self.shares * price
        else:
            self.equity = self.cash

        if self.position != 0:
            days_open = self.current_step - self.position_open_step + 1  # +1 so day-1 counts as 1
            # Unrealised P&L in %, relative to entry
            step_reward = (self.equity - self.entry_equity) / self.entry_equity
            # Per-bar holding cost ()
            # step_reward -= 0.0010 * days_open
        else:
            step_reward = 0.0
...
def train_agent_on_df(agent, df, *, episodes=10, window_size=20,
    ...
    for e in range(episodes):
        ...
        child_log(log_queue, f"[{symbol}] Episode {e + 1}/{episodes} "
                             f"reward={tot_r:.4f} closes={env.close_hits}")
...
def main_loop(agent_params, train_files, test_files, output_dir, *,
    ...
    with multiprocessing.Pool(min(len(args), 3), maxtasksperchild=1) as pool:
        results = pool.map(process_stock, args)
    child_log(log_queue, "Pool finished")

    # ------------------------------------------------------------------
    # Build equal-weight aggregated equity curve
    if not results:
        return
...
if __name__ == "__main__":
    ...
    # File lists ----------------------------------------------------------------
    train_files = [
        r"P:\10_CWP Trade Department\Ryland\unrealai\unrealai\traindata\JPM.csv",
        r"P:\10_CWP Trade Department\Ryland\unrealai\unrealai\traindata\GOOGL.csv",
        r"P:\10_CWP Trade Department\Ryland\unrealai\unrealai\traindata\HON.csv",
    ]
    test_files = [
        r"P:\10_CWP Trade Department\Ryland\unrealai\unrealai\testdata\JPM.csv",
        r"P:\10_CWP Trade Department\Ryland\unrealai\unrealai\testdata\GOOGL.csv",
        r"P:\10_CWP Trade Department\Ryland\unrealai\unrealai\testdata\HON.csv",
    ]
