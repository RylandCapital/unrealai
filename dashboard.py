...
def clean_model_symbols(values: list[object]) -> list[str]:
    ...
    for value in values:
        ...
        if not symbol:
            continue
        if symbol in REPORT_ONLY_BENCHMARK_SYMBOLS:
            continue
...
def make_symbol_exposure_chart(symbol: str, sym_ts: pd.DataFrame) -> go.Figure:
    fig = go.Figure()
    if sym_ts.empty:
        return fig

    dcol = get_date_col(sym_ts)
    pos_col = maybe_col(sym_ts, ["position", "primary_position"])
    if dcol is None or pos_col is None:
        return fig

    exposure_pct = pd.to_numeric(sym_ts[pos_col], errors="coerce") * 100.0
    avg_exposure = float(exposure_pct.dropna().mean()) if exposure_pct.notna().any() else np.nan
    ...
    fig.add_trace(
        go.Bar(
            ...
            y=exposure_pct,
            name="Exposure",
            marker=dict(color="#4f6cff"),
            hovertemplate="%{x|%Y-%m-%d}<br>Exposure: %{y:.0f}%<extra></extra>",
        )
    )
...
def render_symbols(filtered: dict) -> None:
    ...
    st.markdown(f"<div class='chart-title'>{symbol} Exposure</div>", unsafe_allow_html=True)
    st.plotly_chart(make_symbol_exposure_chart(symbol, one_ts), use_container_width=True, config={"displayModeBar": False})
...
def main() -> None:
    ...
    default_benchmark_idx = benchmark_options.index("SPY") if "SPY" in benchmark_options else 0
